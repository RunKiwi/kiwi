...
func TestLeaseEnforcesDAGDependencies(t *testing.T) {
	...
	if ok, err := s.CompleteTask(ctx, TaskCompletion{TaskID: "j1-impl", LeaseID: *l1.LeaseID, FinalStatus: TaskSucceeded}); err != nil || !ok {
...
func TestCompleteTaskFencing(t *testing.T) {
	...
	ok, err := s.CompleteTask(ctx, TaskCompletion{TaskID: "t1", LeaseID: "stale-lease", FinalStatus: TaskSucceeded})
	if err != nil {
	...
	if _, err := s.CompleteTask(ctx, TaskCompletion{TaskID: "t1", LeaseID: *leased.LeaseID, FinalStatus: "BOGUS"}); err == nil {
		t.Error("expected error for invalid final status")
	}

	// Correct token completes.
	ok, err = s.CompleteTask(ctx, TaskCompletion{TaskID: "t1", LeaseID: *leased.LeaseID, FinalStatus: TaskSucceeded, ResultURL: "https://github.com/pr/1", Detail: "detail string"})
	if err != nil {
	...
	ok, _ = s.CompleteTask(ctx, TaskCompletion{TaskID: "t1", LeaseID: *leased.LeaseID, FinalStatus: TaskFailed})
	if ok {
...
func TestRequeueExpiredLeasesAndRefencing(t *testing.T) {
	...
	ok, _ := s.CompleteTask(ctx, TaskCompletion{TaskID: "t1", LeaseID: oldLease, FinalStatus: TaskSucceeded})
	if ok {
...
func TestDependencyFailureCascade(t *testing.T) {
	...
	// impl task fails
	ok, err := s.CompleteTask(ctx, TaskCompletion{TaskID: "j1-impl", LeaseID: *l1.LeaseID, FinalStatus: TaskFailed})
	if err != nil || !ok {

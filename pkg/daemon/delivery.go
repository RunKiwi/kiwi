...
// dashboardURL is the base URL of Kiwi's own dashboard, or "" when the daemon
// was never told one — nothing else could resolve it, since a BYOC daemon
// runs on infrastructure Kiwi doesn't operate.
//
// ee/orchestrator/github_install_api.go reads the same KIWI_DASHBOARD_URL for
// the same reason; duplicated here rather than imported, since pkg/daemon
// (Apache-2.0) must never import ee/ (BSL) — see CLAUDE.md's licensing
// boundary note.
func dashboardURL() string {
	return strings.TrimRight(os.Getenv("KIWI_DASHBOARD_URL"), "/")
}

// taskDrawerURL links back to Kiwi's own record of the run that opened this
// PR, or "" when either half is unavailable — an empty result must never
// become a broken link in a PR body.
func taskDrawerURL(jobID string) string {
	base := dashboardURL()
	if base == "" || jobID == "" {
		return ""
	}
	return base + "/tasks/" + jobID
}

// prTitleAndBody turns a task description, the Architect's own account of the
// finished work, and a link back to Kiwi's record of the run into a pull
...
//
// summary is session.Spec.Summary, written by the Architect specifically to
// serve as this body — see that field's own doc comment in pkg/session/spec.go
// — and is what the title and body are built from when present. Before this,
// both were built from the raw task prompt: the body was "Generated by Kiwi
// worker." followed by the instructions the model was given, never what it
// actually did, and the title named the request rather than the change. A
// task can finish with no summary (an old daemon's checkpoint, a path that
// never reached the Architect's approving review), so both fall back to the
// task's own first line exactly as before rather than producing an empty PR.
func prTitleAndBody(task, summary, taskURL string) (title, body string) {
	...
	first := firstLineOf(summary)
	if first == "" {
		first = firstLineOf(task)
	}
	if first == "" {
	...
	b.WriteString("\n---\n")
	if taskURL != "" {
		fmt.Fprintf(&b, "[Generated by Kiwi](%s)\n", taskURL)
	} else {
		b.WriteString("*Generated by Kiwi*\n")
	}

	body = truncateRunes(b.String(), maxPRBodyLen)
	return title, body
}
...
// git.
func publishResult(ctx context.Context, worktreePath string, spec agent.WorkerSpec, summary, gitToken string, gh githubClient, pushRemoteOverride string) (prURL string, detail string, err error) {
	return publishResultFrom(ctx, worktreePath, spec, summary, gitToken, gh, pushRemoteOverride, "")
}
...
func publishResultFrom(ctx context.Context, worktreePath string, spec agent.WorkerSpec, summary, gitToken string, gh githubClient, pushRemoteOverride, baseSHA string) (prURL string, detail string, err error) {
	...
	base := spec.Ref
	title, body := prTitleAndBody(spec.Task, summary, taskDrawerURL(spec.JobID))

...
- **The Critic's verdict is parsed tolerantly**: the prompt asks for `reasons` as a string and models often answer with a list — one entry per objection, a fair reading of the plural. That was a hard unmarshal error, and since an unparseable verdict fails safe as a rejection, a Critic that was merely formatting differently produced three rejections in a row and failed the task, surfacing a Go type error instead of a review. A list is now joined (every objection reaches the Actor, which needs all of them to fix the edit in one pass), lists of objects are flattened, and an unrecognised shape keeps its raw JSON rather than being discarded. Genuinely malformed output still fails safe.
- **One daemon per org, reused across tasks**: the provisioner names a daemon container by **org**, not by task, so a second submission finds the first one's container already up. It is left alone rather than replaced — the launcher used to open with an unconditional `docker rm -f`, which killed the daemon mid-run whenever a second task arrived. Nothing detects a vanished daemon, so the orphaned task held its lease until the 10-minute TTL lapsed and a retry picked it up: a two-minute task taking twelve, with ten spent on a lease nobody owned. A stopped container is still removed, since it holds the name — as is one left on a **previous image**, but only once the org is idle, since retiring a busy daemon is the very thing that strands the lease. Comparing resolved image IDs rather than tags matters here: the tag is what moves during a deploy, so both sides would read `kiwidaemon:latest` and always look equal.
...
- **A run can be watched while it runs**: the daemon executes the Actor–Critic loop in its own process, so it is the only component that sees a run happen — and it used to keep everything until the end, which made a task that was stuck indistinguishable from one working hard. It now flushes telemetry every few seconds (`POST /api/v1/daemon/progress`), and `GET /api/v1/jobs/{id}/progress` serves the phases completed so far, the command running right now, and the tail of its output. The dashboard renders it with the same timeline component a finished job uses, so nothing shifts when the run ends. Three properties make it safe to ignore: it travels on its own call rather than the lease renewal (a failed progress post must never cost a daemon the task it is running), it is fenced by the lease id like every other task write, and the final result report replaces what was streamed rather than appending to it — otherwise every phase, and the cost attributed to the run, would be counted twice. When the daemon stops reporting, the UI says so; a timestamp that stops advancing is how a hung run tells itself apart from a slow one. Each phase row carries the model that ran it, its duration, tokens and cost, and — in session mode, where the phase encodes the tool as `actor:read_file` — the tool as its own badge, plus **the arguments it was called with**, so a `run` row names the command rather than only showing what it printed. The **Implementer's own model turn is a phase like any other**: it was previously the one thing not timed, so the sum of a task's events came to less than its wall clock and the difference — generation time, and any provider backoff inside it — was attributed to nothing, which made "where did the time go" unanswerable from the record. Events also carry the instant the daemon recorded them, not the instant a flush arrived, because a flush carries several seconds at once and one shared timestamp is exactly the resolution needed to see the gaps between phases. The job drawer sums the same figures into a header strip (models, elapsed, tokens, cost), reading the signed record once it exists and the live feed before then. Absent values are omitted rather than shown as zero: `$0.00` on a job that has not started yet is a false statement, not a neutral one.

  The live feed is also read **once more after a job ends**, and the timeline prefers it over the signed record. The record deliberately carries `detail_hash` rather than `detail` and quotes prose only for the Critic, so for a session run — whose phases are `actor:<tool>` — everything the run said about itself used to disappear the moment it finished. The record remains authoritative about *what* happened and is what the receipt panel reports; it is simply not a transcript.

...
A submit that names no worker model (a zero-config Slack trigger is the common case) defaults to the cheapest Kiwi-funded **economy**-tier model for the Implementer and the cheapest Kiwi-funded **frontier**-tier model for the Architect — same payer, so the split actually happens instead of silently collapsing to one model. An org can opt out of Kiwi-funded defaults entirely (`GET`/`PATCH /api/v1/org/model-source`, or **Settings → Default Model Source**) and fall back to its own connected key instead.

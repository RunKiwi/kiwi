...
- **A run that changes nothing is not a success**: the loop's contract is "green test = done", so if the test command already passes on unmodified code there is correctly nothing to do — which is the normal case for *additive* work, since adding an example does not make `go build` start failing. That used to be reported as SUCCEEDED with no pull request and the words "no changes" as its only explanation: a green tick for work that never happened. Delivery now treats an unchanged worktree as an error, and the task fails with the actual mismatch — that the check passed before any change was made, so the task needs one that fails until the work exists.
...
- **Dependency *changes* are refused, not attempted**: a task targeting `go.mod`, a lockfile or any other manifest cannot be verified here — packages are fetched before the agent runs, so a version it adds afterwards was never downloaded, and verification has no network to fetch it (`go.sum` needs hashes that cannot be computed without the module). That is inherent to verifying model-generated code offline, so the task ends immediately with that explanation instead of spending six Actor steps and the whole budget rediscovering it.

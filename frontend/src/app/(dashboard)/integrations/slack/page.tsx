  ...
  return (
    <div className="p-8 max-w-5xl mx-auto h-full flex flex-col text-stone-900">
      <div className="mb-6">
        <Link
          href="/integrations"
          className="inline-flex items-center gap-1.5 text-xs text-stone-500 hover:text-stone-900 mb-4 transition-colors"
        >
      ...
      <div className="bg-white shadow-2xs border border-sand-200 rounded-2xl p-5 mb-8">
        <h2 className="text-xs font-bold text-stone-400 uppercase tracking-widest mb-4">Add Channel Binding</h2>
        <form onSubmit={onCreate} className="flex flex-col gap-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div>
              <label className="block text-[11px] text-stone-500 mb-1">Slack Workspace / Team ID</label>
              {installations.length > 0 ? (
                <select
                  value={teamID}
                  onChange={(e) => setTeamID(e.target.value)}
                  className="w-full field text-sm bg-zinc-900 border border-sand-200 rounded-lg p-2 text-stone-900"
                  required
                >
                  {installations.map((i) => (
            ...
            <div>
              <label className="block text-[11px] text-stone-500 mb-1">Channel ID</label>
              <input
            ...
            <div>
              <label className="block text-[11px] text-stone-500 mb-1">Repository URL</label>
              <input
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              <label className="block text-[11px] text-stone-500 mb-1">Default Branch / Ref (Optional)</label>
              <input
                placeholder="main"
                value={defaultRef}
            ...
            <div>
              <label className="block text-[11px] text-stone-500 mb-1">Default Test Command (Optional)</label>
              <input
                placeholder="go test ./..."
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              <label className="block text-[11px] text-stone-500 mb-1">
                Worker Model (Optional) <span className="text-stone-400">— leave blank to auto-pick the cheapest available at run time</span>
              </label>
              <input
                placeholder="e.g. claude-haiku-4-5-20251001"
                value={defaultModel}
                onChange={(e) => setDefaultModel(e.target.value)}
                className="w-full field text-sm bg-zinc-900 border border-sand-200 rounded-lg p-2 text-stone-900 font-mono"
              />
            </div>
            <div>
              <label className="block text-[11px] text-stone-500 mb-1">
                Architect Model (Optional) <span className="text-stone-400">— plans &amp; reviews; platform default if blank</span>
              </label>
              <input
          ...
          <div className="flex justify-end mt-2">
            <button
              ...
              className="flex items-center justify-center gap-2 bg-white text-black px-4 py-2 rounded-lg font-medium text-sm disabled:opacity-50 hover:bg-sand-100 transition-colors"
            >
        ...
        {createError && (
          <div className="flex items-center gap-2 text-rose-600 text-sm mt-3">
            <AlertCircle className="w-4 h-4 shrink-0" />
            {createError}
          </div>
        )}
      ...
      <h2 className="text-xs font-bold text-stone-400 uppercase tracking-widest mb-3">Active Bindings</h2>
      {loading ? (
        <div className="flex items-center gap-2 text-sm text-stone-400">
          <Loader2 className="w-4 h-4 animate-spin" /> Loading bindings...
        </div>
      ) : bindings.length === 0 ? (
        <p className="text-stone-400 text-sm">No channels bound yet.</p>
      ) : (
        <div className="flex flex-col gap-3">
          {bindings.map((b) => (
            <div
              key={b.id}
              className="bg-white shadow-2xs p-4 border border-sand-200 rounded-xl flex items-center justify-between group"
            >
              <div className="flex items-center gap-3 min-w-0">
                <div className="w-8 h-8 rounded-lg bg-sand-50 border border-sand-200 flex items-center justify-center shrink-0">
                ...
                <div className="min-w-0">
                  <div className="text-sm font-medium text-stone-900 flex items-center gap-2 flex-wrap">
                    ...
                    <span className="text-stone-700 font-mono text-xs">{b.repo_url}</span>
                  </div>
              ...
              <button
                ...
                className="p-2 text-stone-500 hover:text-rose-600 rounded-lg hover:bg-sand-50 transition-colors disabled:opacity-50"
              >
                {deletingId === b.id ? (
